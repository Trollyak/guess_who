...
d3.csv('data/worlds.csv').then(worlds=>{
    ...
    d3.csv('data/heroes.csv').then(items => {
        ...
        function prepare_game(_seed) {
          ...
          console.log(seed);
          let sorted_items = [];
          const dataGame = items.filter(d=>d['index_world']==_seed.dataset);
          let nameGame = worlds.filter(d=>d['index_world']==_seed.dataset)[0]['name'];
        ...
        if(game){
            ...
            function getSizeWorld(world){
                if(world['index_world']>2){
                    return " ("+items.filter(d=>d['index_world']==world['index_world']).length+")";
                }
                else{
                    return "";
                }
            ...
            function filterGame(g){
                ...
                if (g != 'Все'){
                    ...
                    grid.selectAll("div[class='item']").data(worlds.filter(d=>d['group']==g)).enter()
                        .append("div")
                        .attr('class', 'item')
                    ...
                    grid.selectAll("div[class='item']").data(worlds).enter()
                        .append("div")
                        .attr('class', 'item')
                        .style('background', d => "url("+ d['poster']+")")
                        .style('background-size', 'cover')
            ...
            function clickGame(g){
                console.log(g);
                if (g['name'] === "Создать"){
                    window.open(items[0]['url'], '_blank').focus();
                }
                else if (g['name'] === "Оценить"){
                    window.open(items[1]['url'], '_blank').focus();
                }
                else if (g['name'] === "Помочь проекту"){
                    window.open(home+items[2]['url'], '_blank').focus();
                }
                else if (g['name'] === "Собрать свою коллекцию"){
